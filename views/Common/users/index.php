                    ...
                    <div class="row mb-3">
                        <form method="GET" action="/users" id="searchForm">
                            <div class="row g-2 align-items-end">
                                ...
                                <div class="col-4 col-md-2">
                                    <a href="/users?page=1&per_page=10&sort_by=<?= htmlspecialchars($sortBy ?? 'id') ?>&sort_order=<?= htmlspecialchars($sortOrder ?? 'ASC') ?>" class="btn btn-filter btn-outline-secondary w-100">Reset</a>
                                </div>
                            </div>
                        ...
                        <table class="table table-striped table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th class="th-sortable"><a href="<?= getSortUrl('nomorhp', $sortBy, $sortOrder, $search, $perPage) ?>">Nomor HP</a></th>
                                    <th>Status</th>
                                    <th>Aksi</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr>
                                    <td align="center">
                                        <?php if (!empty($user['picture']) && file_exists(__DIR__ . '/../../../uploads/' . $user['picture'])): ?>
                                        <img src="<?= htmlspecialchars($baseUrl) ?>/uploads/<?= htmlspecialchars($user['picture']) ?>" alt="Profile" class="rounded-circle avatar-sm">
                                        <?php else: ?>
                                        <div class="bg-secondary rounded-circle d-inline-flex align-items-center justify-content-center avatar-sm">
                                            <span class="text-white fw-bold"><?= strtoupper(substr($user['namalengkap'] ?? 'U', 0, 1)) ?></span>
                                        </div>
                                        <?php endif; ?>
                                    </td>
                                    ...
                                    <td align="center"><span class="badge bg-info"><?= ucfirst($user['role'] ?? 'user') ?></span></td>
                                    <td><?= htmlspecialchars($user['nomorhp'] ?? $user['kodesales'] ?? '-') ?></td>
                                    <td align="center">
                                    ...
                                    <td>
                                        <div class="d-flex gap-1">
                                            ...
                                            <a href="/users/delete/<?= $user['id'] ?>" class="btn btn-sm btn-danger" onclick="event.preventDefault(); confirmDelete('Apakah Anda yakin ingin menghapus user <?= htmlspecialchars($user['namalengkap'] ?? '') ?>?', this.href, this); return false;" title="Hapus">
