...
<div class="container">
	...
	<div class="card overflow-visible">
						...
						<div class="d-flex align-items-center mb-3">
							...
							<?php if (!empty($message['sender_picture'])): ?>
								<img src="<?= BASE_URL . $config['upload_url'] . htmlspecialchars($message['sender_picture']) ?>" 
										...
										class="rounded-circle me-3 avatar-md"
										onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
							<?php else: ?>
								<div class="bg-primary text-white rounded-circle d-flex align-items-center justify-content-center me-3 avatar-md">
									<?= $avatarInitial ?>
								</div>
							<?php endif; ?>
							<div>
										...
										<div class="position-relative me-2">
											<?php if (!empty($recipient['recipient_picture'])): ?>
												<img src="<?= BASE_URL . $config['upload_url'] . htmlspecialchars($recipient['recipient_picture']) ?>" 
														...
														class="rounded-circle recipient-avatar avatar-xs"
														onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
												<?php else: ?>
														<div class="bg-secondary text-white rounded-circle d-flex align-items-center justify-content-center recipient-avatar-placeholder avatar-xs" 
															<?= !empty($recipient['recipient_picture']) ? 'style="display: none;"' : '' ?>>
		...
		<div class="card-footer d-flex justify-content-between align-items-center flex-wrap gap-2 overflow-visible position-relative z-1">
			...
			<div class="d-none d-md-flex gap-2">
				...
				$isSender = isset($message['sender_id']) && $message['sender_id'] == ($currentUser['id'] ?? 0);
				$deleteFrom = $isSender ? 'sent' : 'inbox';
				?>
				<a href="/messages/delete/<?= $message['id'] ?>?from=<?= $deleteFrom ?>" class="btn btn-danger" onclick="event.preventDefault(); confirmDelete('Apakah Anda yakin ingin menghapus pesan ini?', this.href, this); return false;">
					<?= icon('trash-can', 'me-1 mb-1', 18) ?> Hapus
				</a>
			</div>
			<!-- Mobile: Dropdown menu -->
			<div class="d-md-none position-relative z-1000">
				<div class="dropdown">
					...
					<ul class="dropdown-menu dropdown-menu-end z-9999" aria-labelledby="actionDropdown">
						<li>
							<a class="dropdown-item" href="/messages/create?reply=<?= $message['id'] ?>">
								<span class="icon-dark"><?= icon('circle-arrow-right', 'me-2 mb-1', 16) ?></span> Balas
							</a>
						</li>
						<li>
							<a class="dropdown-item" href="/messages/create?forward=<?= $message['id'] ?>">
								<span class="icon-dark"><?= icon('share-from-square', 'me-2 mb-1', 16) ?></span> Teruskan
							</a>
						</li>
						<li>
							<a class="dropdown-item" href="#" onclick="window.print(); return false;">
								<span class="icon-dark"><?= icon('print', 'me-2 mb-1', 16) ?></span> Cetak
							</a>
						</li>
						...
						<li>
							...
							$deleteFrom = $isSender ? 'sent' : 'inbox';
							?>
							<a class="dropdown-item text-danger" href="/messages/delete/<?= $message['id'] ?>?from=<?= $deleteFrom ?>" onclick="event.preventDefault(); confirmDelete('Apakah Anda yakin ingin menghapus pesan ini?', this.href, this); return false;">
								<span class="icon-dark"><?= icon('trash-can', 'me-2 mb-1', 16) ?></span> Hapus
							</a>
						</li>
					</ul>
				</div>
