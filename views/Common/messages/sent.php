			...
			<?php else: ?>
				<div class="table-responsive">
					<table class="table table-striped align-middle">
						<thead>
							<tr>
								<th width="45%" class="min-w-200">Subjek</th>
								<th width="30%" class="min-w-150">Penerima</th>
								<th width="15%" class="min-w-100">Tanggal</th>
								<th width="10%" class="min-w-80">Aksi</th>
							</tr>
						</thead>
						<tbody>
									...
									<td class="min-w-80">
										<div class="d-flex gap-1">
											...
											data-bs-toggle="tooltip" data-bs-title="Lihat Pesan">
												<?= icon('show', 'mb-0 mb-1', 16) ?>
											</a>
											<a href="/messages/delete/<?= $message['id'] ?>?from=sent" class="btn btn-danger btn-sm" 
											onclick="event.preventDefault(); confirmDelete('Apakah Anda yakin ingin menghapus pesan ini?', this.href, this); return false;" 
											data-bs-toggle="tooltip" data-bs-title="Hapus Pesan">
												<?= icon('trash-can', 'mb-0 mb-1', 16) ?>
											</a>
										</div>
									</td>
								</tr>
...
document.addEventListener("DOMContentLoaded", function() {
	...
	if (searchForm && searchInput && searchToggleBtn) {
		...
		// Use shared function from messages.js
		const updateButtonStateLocal = function() {
			updateButtonState(isSearchMode, searchToggleBtn, searchIcon, searchForm, searchInput, baseUrl);
		};
