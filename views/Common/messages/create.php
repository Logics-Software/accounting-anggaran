				...
				<div class="row">
					<div class="col-12">
						...
						<div class="mb-3">
							...
							<!-- Users List -->
							<div class="border rounded p-2 max-h-300 overflow-y-auto">
								<div id="usersList">
						...
						<div class="mb-3">
							...
							<div id="quill-editor" class="h-300"></div>
...
document.addEventListener('DOMContentLoaded', function() {
		...
		const usersHtml = users.map(user => {
			...
			return `
				<div class="col-xxl-2 col-xl-2 col-lg-3 col-md-4 col-sm-6 col-12 mb-2">
					<div class="card user-selection-item position-relative ${isSelected ? 'border-primary' : ''} cursor-pointer" data-user-id="${user.id}">
						<div class="position-absolute top-0 start-0 z-10" style="left: 0.25rem;">
							<div class="form-check">
								<input type="checkbox" class="form-check-input border-0" ${isSelected ? 'checked' : ''} onchange="toggleUser(${user.id})">
							</div>
						</div>
						<div class="card-body d-flex align-items-center p-3" style="min-height: 60px;">
							${userPicture}
							<div class="flex-grow-1 ms-2" style="min-width: 0; overflow: hidden;">
								<div class="fw-bold text-truncate small" title="${escapedNamalengkap}">${escapedNamalengkap}</div>
	...
	document.getElementById('messageForm').addEventListener('submit', function(e) {
		...
		const submitBtn = document.querySelector('button[form="messageForm"]') || this.querySelector('button[type="submit"]');
		if (submitBtn && typeof showButtonSpinner === 'function') {
			showButtonSpinner(submitBtn, 'Mengirim...');
		} else if (submitBtn) {
