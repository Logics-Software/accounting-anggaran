                    ...
                    <div class="card-body">
                        <div class="text-center mb-4" id="profile-picture-container">
                            ...
                            <div id="profile-picture-preview">
                                <?php if ($picturePath): ?>
                                <img src="<?= $picturePath ?>" alt="Profile Picture" id="profile-picture-img" class="rounded-circle mb-3 avatar-lg">
                                <?php else: ?>
                                <div id="profile-picture-placeholder" class="bg-secondary rounded-circle d-inline-flex align-items-center justify-content-center mb-3 avatar-lg">
                                    <span class="text-white fw-bold fs-avatar-initial"><?= strtoupper(substr($user['namalengkap'], 0, 1)) ?></span>
                                </div>
                                <?php endif; ?>
                            </div>
                            <div>
                        ...
                        <?php if ($user['nomorhp'] ?? $user['kodesales'] ?? null): ?>
                        <div class="mb-3">
                            <label class="form-label">Nomor HP</label>
                            <input type="text" class="form-control" value="<?= htmlspecialchars($user['nomorhp'] ?? $user['kodesales'] ?? '') ?>" disabled>
                        </div>
                        <?php endif; ?>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof initFormSubmitHandler === 'function') {
        initFormSubmitHandler('form[action="/profile"]', {
