            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex align-items-center">
                        <h4 class="mb-0">Edit Data Jurnal</h4>
                    </div>
                </div>
                <form method="POST" action="/jurnal/edit/<?= $header['id'] ?>" id="jurnalForm">
                <div class="card-body">
                        <h5 class="mb-3">Header Jurnal</h5>
                        ...
                        <div class="row">
                            <div class="col-md-2 mb-3">
                                ...
                                $periodeLabel = '';
                                if (!empty($header)) {
                                    // Extract tahun and bulan from periode (YYYYMM format)
                                    $periodeValue = $header['periode'] ?? '';
                            ...
                            <div class="col-md-2 mb-3">
                                ...
                                <select class="form-select" id="tipejurnal" name="tipejurnal" required>
                                    <?php foreach ($tipeJurnalOptions as $tipe): ?>
                                        <option value="<?= htmlspecialchars($tipe) ?>" <?= ($header['tipejurnal'] ?? '') == $tipe ? 'selected' : '' ?>><?= htmlspecialchars($tipe) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="col-md-2 mb-3">
                                <label for="nojurnal" class="form-label">No. Jurnal <span class="text-danger">*</span></label>
                                <input type="text" class="form-control" id="nojurnal" name="nojurnal" required maxlength="20" value="<?= htmlspecialchars($header['nojurnal'] ?? '') ?>" readonly>
                            </div>
                            <div class="col-md-2 mb-3">
                                <label for="tanggaljurnal" class="form-label">Tanggal Jurnal <span class="text-danger">*</span></label>
                                <?php
                                $tanggalJurnal = $header['tanggaljurnal'] ?? date('Y-m-d');
                                $tanggalJurnalFormatted = date('Y-m-d', strtotime($tanggalJurnal));
                                ?>
                                <input type="date" class="form-control" id="tanggaljurnal" name="tanggaljurnal" required value="<?= htmlspecialchars($tanggalJurnalFormatted) ?>">
                            </div>
                            <div class="col-md-3 mb-3">
                                <label for="noreferensi" class="form-label">No. Referensi</label>
                                <input type="text" class="form-control" id="noreferensi" name="noreferensi" maxlength="50" value="<?= htmlspecialchars($header['noreferensi'] ?? '') ?>" placeholder="Masukkan nomor referensi">
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                ...
                                <select class="form-select" id="posting" name="posting">
                                    <?php foreach ($postingOptions as $posting): ?>
                                        <option value="<?= htmlspecialchars($posting) ?>" <?= ($header['posting'] ?? 'NON POSTING') == $posting ? 'selected' : '' ?>><?= htmlspecialchars($posting) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="col-md-4 mb-3" id="nomor_posting_container" style="display: <?= ($header['posting'] ?? 'NON POSTING') != 'NON POSTING' ? 'block' : 'none' ?>;">
                                <label for="nomor_posting" class="form-label">Nomor Posting</label>
                                <input type="text" class="form-control" id="nomor_posting" name="nomor_posting" maxlength="20" value="<?= htmlspecialchars($header['nomor_posting'] ?? '') ?>" placeholder="Masukkan nomor posting">
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-12 mb-3">
                                ...
                                <textarea class="form-control" id="keterangan" name="keterangan" rows="2" maxlength="254" required placeholder="Masukkan Uraian"><?= htmlspecialchars($header['keterangan'] ?? '') ?></textarea>
                            </div>
                        </div>
                            ...
                            <table class="table table-bordered" id="detailTable">
                                ...
                                <tbody id="detailBody">
                                    <?php if (!empty($details)): ?>
                                        ...
                                        <tr class="detail-row">
                                            <td><?= $index + 1 ?></td>
                                            <td>
                                                <select class="form-select akun-select" name="details[<?= $index ?>][id_perkiraan]" required>
                                                    <option value="">Pilih Akun</option>
                                                    <?php foreach ($allAkun as $akun): ?>
                                                        <option value="<?= $akun['id'] ?>" data-nomor="<?= htmlspecialchars($akun['nomor_akun']) ?>" data-nama="<?= htmlspecialchars($akun['nama_akun']) ?>" <?= ($detail['id_perkiraan'] ?? 0) == $akun['id'] ? 'selected' : '' ?>>
                                            ...
                                            <td>
                                                <input type="text" class="form-control" name="details[<?= $index ?>][uraian]" maxlength="254" value="<?= htmlspecialchars($detail['uraian'] ?? '') ?>" placeholder="Uraian">
                                            </td>
                                            <td>
                                                <input type="text" class="form-control text-end debet-input" name="details[<?= $index ?>][debet]" value="<?= number_format(round($detail['debet'] ?? 0), 0, ',', '.') ?>" placeholder="0">
                                            </td>
                                            <td>
                                                <input type="text" class="form-control text-end kredit-input" name="details[<?= $index ?>][kredit]" value="<?= number_format(round($detail['kredit'] ?? 0), 0, ',', '.') ?>" placeholder="0">
                                            </td>
                                            <td>
                                ...
                                <tfoot>
                                    <tr>
                                        ...
                                        <td class="text-end fw-bold" id="totalDebet"><?= number_format($header['totaldebet'] ?? 0, 2, '.', '') ?></td>
                                        <td class="text-end fw-bold" id="totalKredit"><?= number_format($header['totalkredit'] ?? 0, 2, '.', '') ?></td>
                                        <td>
                    ...
                    <div class="card-footer d-flex justify-content-between">
                        ...
                        <button type="submit" class="btn btn-primary">
                            <?= icon('save', 'me-1 mb-1', 18) ?>Update Jurnal
                        </button>
                    </div>
                </form>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Show/hide nomor_posting based on posting (only if posting dropdown exists)
    const postingSelect = document.getElementById('posting');
    const nomorPostingContainer = document.getElementById('nomor_posting_container');
    
    if (postingSelect && nomorPostingContainer) {
        postingSelect.addEventListener('change', function() {
            if (this.value !== 'NON POSTING') {
                nomorPostingContainer.style.display = 'block';
            } else {
                nomorPostingContainer.style.display = 'none';
                document.getElementById('nomor_posting').value = '';
            }
        });
        
        // Initialize on page load
        if (postingSelect.value !== 'NON POSTING') {
            nomorPostingContainer.style.display = 'block';
        } else {
            nomorPostingContainer.style.display = 'none';
        }
    ...
    document.getElementById('addRow').addEventListener('click', function() {
        ...
        newRow.innerHTML = `
            ...
            <td>
                <input type="text" class="form-control" name="details[${rowCount - 1}][uraian]" maxlength="254" required placeholder="Uraian">
            </td>
            <td>
    ...
    function attachRowEvents(row) {
        ...
        if (debetInput) debetInput.addEventListener('input', calculateTotals);
        if (kreditInput) kreditInput.addEventListener('input', calculateTotals);
    ...
    // Attach events to existing rows and format existing values
    document.querySelectorAll('.detail-row').forEach(row => {
        attachRowEvents(row);
        // Format existing values in debet/kredit inputs
        const debetInput = row.querySelector('.debet-input');
        const kreditInput = row.querySelector('.kredit-input');
        if (debetInput && debetInput.value) {
            const numValue = unformatNumber(debetInput.value);
            if (numValue > 0) {
                debetInput.value = formatNumber(numValue);
            }
        }
        if (kreditInput && kreditInput.value) {
            const numValue = unformatNumber(kreditInput.value);
            if (numValue > 0) {
                kreditInput.value = formatNumber(numValue);
            }
        }
    });
