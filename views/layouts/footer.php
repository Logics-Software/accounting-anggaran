    ...
    ?>
    <script src="<?= htmlspecialchars(\Core\Helpers\AssetHelper::js('bootstrap.bundle.min.js', $baseUrl)) ?>"></script>
    <script src="<?= htmlspecialchars(\Core\Helpers\AssetHelper::js('form-submit-handler.js', $baseUrl)) ?>"></script>
    ...
    if ($needsDownloadHandler) {
        // VIEW OPTIMIZATION: Use AssetHelper for versioning
        echo '<script src="' . htmlspecialchars(\Core\Helpers\AssetHelper::js('download-handler.js', $baseUrl)) . '"></script>';
    }
    
    // Messages module script - load on message pages
    $messagePages = ['/messages/'];
    $needsMessagesJS = false;
    
    foreach ($messagePages as $page) {
        if (strpos($currentPath, $page) !== false) {
            $needsMessagesJS = true;
            break;
        }
    }
    ...
    if ($needsMessagesJS) {
        // VIEW OPTIMIZATION: Use AssetHelper for versioning
        echo '<script src="' . htmlspecialchars(\Core\Helpers\AssetHelper::js('messages.js', $baseUrl)) . '"></script>';
    }
    ?>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form id="appConfirmDynamicForm" class="mt-2 d-none" novalidate></form>
                </div>
    ...
    (function () {
        ...
        window.confirmDelete = function (message, url, buttonElement) {
            return showConfirmModal({
                ...
                onConfirm: function () {
                    if (url) {
                        // Show spinner on the delete button if provided
                        if (buttonElement && typeof showButtonSpinner === 'function') {
                            const btn = typeof buttonElement === 'string' 
                                ? document.querySelector(buttonElement) 
                                : buttonElement;
                            if (btn) {
                                showButtonSpinner(btn, 'Menghapus...');
                            }
                        }
