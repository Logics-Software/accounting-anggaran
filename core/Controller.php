...
class Controller {
    ...
    protected function view($view, $data = []) {
        // FIX: Don't close session in view() because flash messages might be read during rendering
        // Session will be closed automatically by PHP at end of script
        
        // VIEW OPTIMIZATION: Use output buffering for better performance
        // Single buffer operation reduces overhead and allows compression
        
        // Support both old format (dashboard/index) and new format (Common/dashboard/index)
        // Also support backward compatibility for old paths
        $viewPaths = [];
        
        // New format with namespace (Common/dashboard/index)
        if (strpos($view, 'Common/') === 0 || strpos($view, 'Common\\') === 0) {
            $viewPath = str_replace('\\', '/', $view);
            $viewPaths[] = __DIR__ . '/../views/' . $viewPath . '.php';
        } else {
            // Try new format first (Common/dashboard/index)
            $viewPaths[] = __DIR__ . '/../views/Common/' . $view . '.php';
            // Then try old format for backward compatibility (dashboard/index)
            $viewPaths[] = __DIR__ . '/../views/' . $view . '.php';
        }
        
        foreach ($viewPaths as $viewFile) {
            if (file_exists($viewFile)) {
                // VIEW OPTIMIZATION: Use output buffering for efficient rendering
                // This reduces overhead and allows better control over output
                ob_start();
    ...
    protected function redirect($url) {
        // OPTIMIZATION: Close session before redirect
        // Session data is already written, no need to keep it open
        Session::writeClose();
        header("Location: {$url}");
        exit;
    }
