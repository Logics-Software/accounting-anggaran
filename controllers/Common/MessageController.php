...
use Auth;
use Session;
use Models\Common\MessageModel;
...
class MessageController extends Controller {
	...
	public function index() {
		...
		$this->view('Common/messages/index', [
			'title' => 'Pesan Masuk',
			'messages' => $messages,
	...
	public function sent() {
		...
		$this->view('Common/messages/sent', [
	...
	public function create() {
		...
		$this->view('Common/messages/create', [
			'title' => 'Tulis Pesan',
			'users' => $users,
	...
	public function show($id) {
		...
		$this->view('Common/messages/show', [
			'title' => 'Detail Pesan',
			'messageData' => $message,
	...
	public function delete($id = null) {
		...
		try {
			// Determine redirect URL based on 'from' parameter or referer
			$from = $_GET['from'] ?? $_POST['from'] ?? null;
			$redirectUrl = '/messages'; // Default to inbox
			
			// Check referer if 'from' parameter is not provided
			if (!$from && isset($_SERVER['HTTP_REFERER'])) {
				$referer = $_SERVER['HTTP_REFERER'];
				if (strpos($referer, '/messages/sent') !== false) {
					$from = 'sent';
				} else {
					$from = 'inbox';
				}
			...
			// Set redirect URL based on 'from' parameter
			if ($from === 'sent') {
				$redirectUrl = '/messages/sent';
			} else {
				// Also check if message is from sent by checking if user is sender
				$message = $this->messageModel->findById($messageId);
				if ($message && ($message['sender_id'] ?? 0) == $userId) {
					// User is sender, might be from sent page
					// But we'll use the 'from' parameter if available
					if ($from !== 'inbox') {
						$redirectUrl = '/messages/sent';
					}
				}
			...
			if ($result) {
				...
				Session::flash('error', 'Gagal menghapus pesan');
				$this->redirect($redirectUrl);
			}
		} catch (Exception $e) {
			Session::flash('error', 'Terjadi kesalahan: ' . $e->getMessage());
	...
	public function search() {
		...
		$this->view('Common/messages/search', [
