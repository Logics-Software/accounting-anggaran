...
class LoginLog {
    ...
    public function markLogout($sessionToken) {
        ...
        // Return number of affected rows to check if update was successful
        return $stmt->rowCount() > 0;
    }
    
    /**
     * Mark logout by user_id (fallback if session_token doesn't match)
     * Updates the most recent login record for the user that hasn't been logged out
     */
    public function markLogoutByUserId($userId, $sessionToken = null) {
        if (empty($userId)) {
            return false;
        }

        // First, find the most recent login record ID for this user
        $findSql = "SELECT id FROM login_log 
                    WHERE user_id = ? 
                    AND logout_at IS NULL 
                    AND status = 'success'
                    ORDER BY login_at DESC 
                    LIMIT 1";
        $findResult = $this->db->fetchOne($findSql, [$userId]);
        
        if (!$findResult || empty($findResult['id'])) {
            return false;
        }
        
        // Then update that specific record
        $sql = "UPDATE login_log 
                SET logout_at = ?, status = 'logout', updated_at = NOW() 
                WHERE id = ?";
        $params = [date('Y-m-d H:i:s'), $findResult['id']];
        $stmt = $this->db->query($sql, $params);
    ...
    public function countToday() {
        ...
        // Generate cache key
        $cacheKey = "login_log_count_today_{$today}";
        
        // Try cache first (5 minutes TTL - data berubah setiap hari)
        ...
        // Cache for 5 minutes (300 seconds) - data berubah setiap hari jadi TTL pendek cukup
