...
class MessageModel {
	...
	public function markAsRead($messageId, $userId) {
		...
		$result = $this->db->query($sql, [$messageId, $userId]);
		
		// Invalidate unread count cache for this user
		Cache::delete("message_unread_count_{$userId}");
	 ...
	 * Cached for 30 seconds (short TTL karena sering berubah)
	 */
	public function getUnreadCount($userId) {
		// Generate cache key
		$cacheKey = "message_unread_count_{$userId}";
		
		// Try cache first (short TTL - 30 seconds karena sering berubah)
		...
		// Cache for 30 seconds (short TTL karena sering berubah)
